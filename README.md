...
npm run start:api:3333  # http://localhost:3333/api (only if 3333 is free)
```

After `npm run build:user`, sync native shells from `apps/user-app`:

```sh
npm run cap:sync:user
```

Product architecture notes live in [docs/two_apps_architecture_plan.md](docs/two_apps_architecture_plan.md).

---

✨ Your [Nx workspace](https://nx.dev) is ready ✨.
